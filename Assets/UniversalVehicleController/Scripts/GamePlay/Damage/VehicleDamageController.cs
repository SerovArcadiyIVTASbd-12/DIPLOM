...
#pragma warning restore 0649
        ...
        public void OnCollision (Collision col, bool fromStayCollistion)
        {
            ...
            //Debug.Log("Вывод импульса " + impulse);
            if (Mathf.Max (relativeVelocityMagnitude, fromStayCollistion ? impulse : 0) * DamageFactor > 1)
            {
                ...
                for (int i = 0; i < contactsCount; i++)
                {
                    ...
                    Vector3 localImpactPoint = transform.InverseTransformPoint(contact.point);
                    //Debug.Log($"💥 Удар! Локальные координаты: {localImpactPoint}");
                    //Debug.Log("Относительная скорость " + relativeVelocityMagnitude);
                    //Debug.Log("Вывод скорости " + relativeVelocityMagnitude.ToInt());
                    //float impactSpeedFromImpulse = (col.impulse.magnitude * 3.6f);
                    //Debug.Log($" Скорость удара по импульсу: {impactSpeedFromImpulse} км/ч");
            
                    float carSpeed = RB.velocity.magnitude;

                    PG.DamageHuman.setCoordinate( localImpactPoint);
                    //PG.DamageHuman.testing();
                ...
                //Vector3 velocityBeforeImpact = RB.velocity;  // Скорость до столкновения
                
                //Vector3 velocityAfterImpact = new Vector3(0, 0, 0); // Скорость после столкновения (может быть нулевая или измененная)
                //PG.DamageHuman.transformToG(velocityBeforeImpact, velocityAfterImpact);
